...
- The rotor consists of a wheel with internal wiring connecting 1 of 26
  **input signals** to 1 of 26 **output signals**. The wiring can be represented by a
...
- When a rotor rotates, the wiring of the rotor changes. The input AND the
  output both move 1 letter "down". Moving the input 1 letter down is
...
- To set the position of a rotor, each rotor is represented by a string of letters in
  random order. As a simplified example, we can apply the letters `ABCDE`, which we'll
  call the **rotor string**. The first letter of this rotor string is visible on the
  operator's keyboard, and can be used to set the position of the rotor. We can now use
  this setting to describe a rotor by its rotor string and connection string,
  for example: `ABCDE BDEAC`
- When rotating the rotor, the first letter is removed from the front of the string,
  and is now put at the back of the string, so rotating a rotor with rotor string
...
- The "rotor setting" is a 1-based index that determines the first letter in the
  ...
  example above, we would shift the letters `ABCDE` by 1 position before connecting
  them, so the rotor would now be represented by `BCDEA EDABC` So even though this the
  same rotor as in the example a above, while the `B` is visible to the user, the
  internal wiring is the same as if the rotor was set to an `A` with rotor position 1.
- Each rotor has a number of "notches". When a rotor passes a notch by being rotated,
...
3. Implement getting the current position of the rotor by its visible letter:
   `get_position() -> str`
4. Implement the `encode` function: encodes a single character based on the current
   rotor settings. The direction of the encoding should also be supported. E.g. given
   rotor connections `BDEAC`, encoding "inward" (i.e. input to output) would  encode
   `A` to `B`: `assert encode("A", direction="in") == "B"`, similarly, encoding
...
5. Implement the `rotate()` function: Imagine that a rotor has a connection that
...
7. Optional: implement notches. When a rotor rotates past a letter with a notch,
   ...
   Update your unit tests. Also, update the `rotate()` function to now return whether
   the next rotor should also rotate, for example by returning a bool. Hint: it may be
